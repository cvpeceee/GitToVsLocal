...
def from_columns(mapping, fp):
    ...
    onlyname = os.path.splitext(base)[0]
    finalname = str.encode(onlyname.ljust(8))
    ...
    fields = OrderedDict()
    position = 0
    for labelName, column in columns.items():
        label = labelName.split('#')[1].encode('ISO-8859-1')
        n = labelName.split('#')[0].encode('ISO-8859-1')

        # name must be exactly 8 bytes and usually is alphanumeric
        name = b'_'.join(re.findall(b'[A-Za-z0-9_]+', n))[:8].ljust(8)
